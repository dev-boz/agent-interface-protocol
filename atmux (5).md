...
- **Minimal footprint**: Overstory has 36 CLI commands and a SQLite mail system. ATMUX is 8 MCP tools and tmux commands.
...
One MCP server, installed on every CLI agent. This is the complete agent coordination protocol — it replaces both ACP and Claude Code's native TeammateTool. If an agent is Claude Code, it uses atmux-mcp, not `sendMessage`/`readMessages`. One protocol for all agents, regardless of vendor.
...
notify
  - args: target_agent (or "all"), message, priority (high | medium | low)
  - appends: workspace/events.jsonl (event type "notify")
  - purpose: direct agent-to-agent communication, mid-task
```

### How `notify` Works

This is the impetus mechanism. Agents with high interest in each other use `notify` to communicate directly during work — not just when finished.

Example: two architects working on related systems:
...
The receiving agent sees this on its next check of the event log because its interest map has the sender as "high." A coder with low interest in architects would skip right past these messages.

This is what makes high-impetus agents (architects, tech leads) collaborate in real time while low-impetus agents (coders, testers) stay focused on their own tasks without noise.
...
### That's it. Eight tools. This IS your agent coordination protocol.

### Recursive Spawning

`spawn_teammate` turns ATMUX from a flat orchestrator into a self-organising agent tree. Any agent can spawn sub-agents, and those sub-agents can spawn their own. The task structure dictates the team structure — no pre-configuration needed.
...
```json
{
  ...
  "review-mgr": {
    ...
    "tmux_window": "atmux:review-mgr"
  },
  "security-rev": {
    "depth": 2, "parent": "review-mgr",
    "children": ["cred-checker"],
    "tmux_window": "atmux:security-rev"
  },
  "cred-checker": {
    "depth": 3, "parent": "security-rev",
    "children": [],
    "tmux_window": "atmux:cred-checker"
  }
}
```
...
Claude Code has a native teammate system with `sendMessage` and `readMessages`. ATMUX's atmux-mcp replaces it entirely:

- **TeammateTool is Claude-only**. atmux-mcp works on any CLI agent.
- **TeammateTool is hub-and-spoke**. atmux-mcp supports recursive trees.
- **TeammateTool has no task queue**. atmux-mcp has atomic claiming.
- **TeammateTool has no interest maps**. atmux-mcp has impetus-driven subscriptions.
- **TeammateTool has no event log**. atmux-mcp has full audit trail.
- **TeammateTool has no direct peer messaging**. atmux-mcp has `notify` for agent-to-agent collaboration.

If an agent happens to be Claude Code, it uses atmux-mcp — not the native TeammateTool. One protocol for all agents.
...
| Claude Code TeammateTool | atmux-mcp (vendor-neutral, richer semantics) |
...
`atmux dashboard` — a live multiplexed view of every agent working in real time. The primary view is a split-pane grid showing actual tmux pane output: streaming text, thinking, tool calls, spinning progress indicators, errors. Eight agents churning away simultaneously in an 8-way split.
...
ATMUX teammates are the heavy delegation tier. Two companion tools provide lighter tiers:

**Universal CLI MCP** (in progress) — any agent can call any CLI headlessly as a one-shot function. `cli_oneshot("gemini", "lint this code", files=["main.py"])` spawns a process, gets the result, terminates immediately. Zero persistence, zero RAM overhead. Any ATMUX teammate can use this as an MCP tool for quick cross-model calls.

**CLI-Agnostic Sub-Agent Wrapper** (planned) — normalises native sub-agent APIs across CLIs. Claude Code has sub-agents (Oracle, Specialist), Amp has sub-agents (Oracle, Librarian, Scribe), others don't. The wrapper provides a single `use_subagent()` interface that uses native sub-agents where available and falls back to one-shot calls where not.

Together with ATMUX teammates, these form a three-tier delegation hierarchy: heavy (persistent teammates), medium (CLI sub-agents), light (one-shot calls). Right tool for the right job.
